...
class MaximalSum
{
    static void Main()
    {
        ...
        {
            if (sum <= 0)   //if previous cycle`s current sum was less than or equal to zero, then restart calculating it from the current indx member. 
                // This is true because no matter the next number, the current sum will either be smaller if next num is negative or it wouldn`t be bigger 
                // than the next number if it is positive. The next positive number alone will be bigger than the current sum, so we just need to restart calculating it.
            {
